...
class IsPartOfConversation(permissions.BasePermission):
    message = _('you do not have access to this conversation')

    def has_object_permission(self, request, view, conversation):
        if request.user.is_authenticated:
            return conversation.contact_participant == request.user or conversation.starter_participant == request.user
        else:
            return False


class IsAMessageContact(permissions.BasePermission):
    message = _('you do not have access to this message')

    def has_object_permission(self, request, view, message):
        if request.user.is_authenticated:
            return message.contact == request.user
        else:
            return False
